...
router.post('/register', async (req, res, next) => {
  try {
    ...
    const exists = await db('customers').whereRaw('LOWER(email)=LOWER(?)', [email]).first();
...
router.post('/login', async (req, res, next) => {
  try {
    ...
    const user = await db('customers').whereRaw('LOWER(email)=LOWER(?)', [email]).first();
    if (!user) return res.status(401).json({ error: 'Invalid credentials' });
    const ok = await bcrypt.compare(password, user.password_hash);
    if (!ok) return res.status(401).json({ error: 'Invalid credentials' });
    const token = signToken({ id: user.id, email: user.email });
    const { id, name } = user;
    res.json({ token, customer: { id, name, email: user.email, created_at: user.created_at } });
  } catch (e) { next(e); }
});
